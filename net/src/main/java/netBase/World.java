...
public class World<T extends GameClient>
{
	...
	/*ConcurrentHashMap*/
	protected static ConcurrentHashMap<Integer, GameClient> channelMap = new ConcurrentHashMap<Integer, GameClient>();
	...
	public void addChannel(GameClient connection)
	{
		...
		StringBuilder builder = new StringBuilder("新连接建立，连接ID[").append(connection.getChannel().getId());
		builder.append("]连接地址[")
		.append(connection.getChannel().getRemoteAddress().toString()).append("]");
